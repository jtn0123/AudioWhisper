...
final class DataManagerTests: XCTestCase {
    ...
    func testPaginatedFetchRecords() async throws {
        ...
        for index in 0..<10 {
            let record = TranscriptionRecord(
                text: "Test record \(index) with searchable content",
                provider: .local,
    ...
    func test_fetchRecords_paginatesAndFiltersBySearch() async throws {
        ...
        for index in 0..<10 {
            let text = (index % 2 == 0) ? "hello entry \(index)" : "world entry \(index)"
            let record = TranscriptionRecord(text: text, provider: .local, duration: Double(index))
            try await dataManager.saveTranscription(record)
        }
